...
void setup() {
  //size(500, 500);
  fullScreen();
  String doc = loadStrings("PI/"+docs[indc])[0];
  ...
  for (int index = 0; index < pi.length-1; index++) {
    ...
    stroke((index == 0)? color(255, alph) : coloresA[digit1]);
  ...
  for (int i = 0; i < 10; i++) if (mayor <= rep[i]) mayor = i;

  piImg = loadImage("png/PI.png");
  tint(colores[mayor]);
  image(piImg, -radius-100, 0-50, 100, 100);
  ...
  fill(colores[mayor]);
  textSize(20);   
  text(cont, -radius-90, 0-60);
  text("decimals", -radius-90, 75);
  
  saveFrame("SS/"+cont+"PI.jpg");
}
